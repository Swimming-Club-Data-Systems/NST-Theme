            ...
            <?php if (have_posts()) : while (have_posts()) : the_post(); ?>
                <div class="blog-main">
                  ...
                  <?php the_author(); ?>
                            ...
                            <p class="mb-0">
                                <time id="time" class="published blog-post-meta" datetime="<?php the_time('c'); ?>"><i
                                            class="fa fa-clock" aria-hidden="true"></i> <abbr
                                            title="<?php the_time('c'); ?>"><span id="dtOut"><noscript><?php the_time('j F Y'); ?></noscript></span></abbr>
                                </time>
                                -
                                <?php the_category(', '); ?>
                            </p>
                            <?php if (get_the_time('U') < strtotime('-3 months')) { ?>
                                <div class="d-inline-block p-3 mt-3 bg-warning text-dark rounded">
                                    <i class="fa fa-clock" aria-hidden="true"></i> This post is more than
                                    <strong><?= htmlspecialchars(human_time_diff(get_the_time('U'))) ?> old</strong>
                                </div>
                            <?php } ?>
                        </div>
